#!/usr/bin/env python3
"""
Docker TCP Relay v3.0
Listen TCP 2376 → forward to unix:/var/run/docker.sock
Fixes Docker 29.4+ MinAPIVersion=1.40 vs Traefik HTTP negotiate 1.24

Key fix: Rewrite ALL HTTP requests on persistent connections (HTTP/1.1)
"""
import socket
import threading
import re
...
def process_client_data(data):
    """Rewrite all HTTP requests' API version in data chunk"""
...
def main():
    ...
    server.listen(200)
    print(f"Docker TCP Relay v3.0 on 0.0.0.0:{TCP_PORT} → {DOCKER_SOCKET}", flush=True)
