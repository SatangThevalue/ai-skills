...
Use standard Python logging to capture execution flow, but configure the FastMCP logger if you need to debug or display raw communication messages exchanged between the server and client.

```python
import logging
from fastmcp.utilities.logging import get_logger, configure_logging

# Configure system-wide logging level for FastMCP
configure_logging(level="INFO")
logger = get_logger("fastmcp.server.context.to_client")
logger.setLevel(logging.DEBUG)  # Displays raw communication with the client on stdout/stderr
```

---

## 5. Configuration & Declarative Setup

### A. FastMCP Declarative Configuration (fastmcp.json)
From version `2.12.0` and above, FastMCP supports a standard configuration file named `fastmcp.json`. This declarative approach avoids typing long lists of CLI arguments and simplifies deployment configurations.

An example of `fastmcp.json` structure:
```json
{
  ...
  "env": {
    "ACME_API_KEY": {
      "description": "API Key for accessing Acme REST endpoints",
      "required": true
    }
  },
  "settings": {
...
### B. Test the Server Before Integrating It Anywhere
