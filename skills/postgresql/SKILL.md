...
- **`pgvector`**: vector similarity search for embeddings. When deploying via docker, use `pgvector/pgvector:pg18` (or corresponding version). Activate with `CREATE EXTENSION IF NOT EXISTS vector;`.
...
When deploying PostgreSQL 18+ via Docker, do NOT mount directly to `/var/lib/postgresql/data` (can block `pg_upgrade`). Mount to `/var/lib/postgresql` or set `PGDATA` to a subdirectory:
```yaml
environment:
